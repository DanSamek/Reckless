...
fn eval_correction(td: &ThreadData, ply: isize) -> i32 {
    ...
    let king_square = td.board.king_square(stm);
    ...
    (corrhist.pawn.get(stm, td.board.pawn_key())
        ...
        + corrhist.segment[td.board.segment_index(king_square)].get(stm, td.board.segment_key(king_square))
...
fn update_correction_histories(td: &mut ThreadData, depth: i32, diff: i32, ply: isize) {
    ...
    let king_square = td.board.king_square(stm);
    corrhist.segment[td.board.segment_index(king_square)].update(stm, td.board.segment_key(king_square), bonus);
